locals {
  create_storage_cdn = var.storage_domain_name != "" && var.route53_zone_id != ""
}

# ACM certificate for storage domain (must be in us-east-1 for CloudFront).
resource "aws_acm_certificate" "storage" {
  count             = local.create_storage_cdn ? 1 : 0
...
resource "aws_route53_record" "storage_cert_validation" {
  for_each = local.create_storage_cdn ? {
    for dvo in aws_acm_certificate.storage[0].domain_validation_options : dvo.domain_name => {
...
resource "aws_cloudfront_distribution" "storage" {
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate_validation.storage[0].certificate_arn
...
resource "aws_route53_record" "storage" {
  ...
  alias {
    ...
    zone_id                = aws_cloudfront_distribution.storage[0].hosted_zone_id
