...
func TestError(t *testing.T) {
	...
	if env.Success || env.Error == nil || env.Error.Code != "BAD_REQUEST" || env.Message != "bad" || env.Path != "/v1/things" {
		t.Errorf("unexpected envelope: %+v", env)
	}
	if env.Error == nil || env.Error.Context == nil {
		t.Error("error.context must be present")
	}
	if env.Error == nil || env.Error.Type == "" {
		t.Error("error.type must be set")
	}
}
