		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if err != nil {
					...
					if err := os.Remove(tt.outputPath); err != nil && !os.IsNotExist(err) {
						t.Logf("cleanup remove %s: %v", tt.outputPath, err)
					}
				}
			}
		})
...
func TestContextCancellation(t *testing.T) {
	...
	defer func() {
		if err := os.Remove(tmpFile); err != nil && !os.IsNotExist(err) {
			t.Logf("cleanup remove tmpFile: %v", err)
		}
	}()
	...
	defer func() {
		if err := os.RemoveAll(outputDir); err != nil && !os.IsNotExist(err) {
			t.Logf("cleanup removeAll outputDir: %v", err)
		}
	}()
...
func TestCountSegments(t *testing.T) {
	...
	defer func() {
		if err := os.RemoveAll(tmpDir); err != nil && !os.IsNotExist(err) {
			t.Logf("cleanup removeAll tmpDir: %v", err)
		}
	}()
