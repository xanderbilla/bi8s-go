...
func (s *EncoderService) processEncodingJobFromFile(ctx context.Context, jobPtr *model.EncoderJob, tempFile string) {
	...
	workDir, err := os.MkdirTemp(utils.TmpDir(), fmt.Sprintf("bi8s-work-%s-*", job.JobID))
	if err != nil {
	...
	defer func() {
		if err := os.RemoveAll(workDir); err != nil && !os.IsNotExist(err) {
			logger.WarnContext(ctx, "failed to remove working directory", "job_id", job.JobID, "work_dir", workDir, "error", err.Error())
		}
	}()
...
func (s *EncoderService) processVideoParallel(ctx context.Context, job model.EncoderJob, inputFile, workDir string) model.EncoderJob {
	...
	ffmpegSem := make(chan struct{}, s.ffmpegPerJob)
	acquireFFmpeg := func() func() {
		ffmpegSem <- struct{}{}
	...
	if len(videoQualities) > 0 {
		...
		if err := utils.GenerateMasterPlaylist(localRecordMasterPath, recordQualities, []utils.AudioPlaylist{}); err != nil {
			...
			if err := s.uploadFile(ctx, localRecordMasterPath, s3RecordPath); err != nil {
				job.Errors = append(job.Errors, model.EncoderError{
					Code:    "MASTER_PLAYLIST_UPLOAD_FAILED",
					Message: "Failed to upload record master playlist",
					Stage:   "master_playlist_upload",
					Details: err.Error(),
				})
			} else if err := s.uploadFile(ctx, localCurrentMasterPath, s3CurrentPath); err != nil {
				job.Errors = append(job.Errors, model.EncoderError{
					Code:    "MASTER_PLAYLIST_UPLOAD_FAILED",
					Message: "Failed to upload current master playlist",
					Stage:   "master_playlist_upload",
					Details: err.Error(),
				})
			} else {
